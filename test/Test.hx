	...
	final args = {
		...
		switch(args.slice(0, 2)) {
			case [sillyScriptExecutable, unitTestFolder]: {
				executable: sillyScriptExecutable,
				tests: unitTestFolder,
				updateIntended: args.contains("--update-intended")
			}
			case _: {
				error("haxe --run Test.hx <path to SillyScript executable> <path to unit tests>");
				return null;
			}
		}
	};

	if(!FileSystem.exists(args.executable)) {
...
function runTests(args: Args) {
	for(testFolder in FileSystem.readDirectory(args.tests)) {
		...
		switch(runTest(path, args.executable)) {
			...
			case TestFailedButDoesntMatch(output) if(args.updateIntended): {
...
function runTest(folder: String, executable: String): RunTestResult {
	final sillyFiles = [];
	...
	if(sillyFiles.length > 0) {
		...
		switch(run(executable, [sillyPath, outputPath, "--dont-color-errors"])) {
			case Success: {
